    ...
    return (
        <section className='flex h-full '>
            <div className='flex flex-col w-full items-center gap-7'>
                <h1 className='italic font-bold underline text-xl my-4'>새로운 제품 등록</h1>
                <div className='w-full flex flex-col items-center gap-7'>
                    {file && <img className="w-96" src={URL.createObjectURL(file)} alt='미리보기이미지'/>}
                    <form onSubmit={handleSubmit} className='flex flex-col gap-6 w-2/3'>
                        <input 
                            className="border-black w-full border border-slate-400 h-10 w-2/4 border-0" type="file"
                            accept='image/*' 
                            name="file"
                            required
                        ...
                        />
                        {success && <p className='my-2 italic font-bold underline'>✅{success}</p>}
                        <Button text={isUploadging ? '업로드중...' : '제품 등록하기'} disabled={isUploadging}/>
                    </form>
                </div>
            </div>
        </section>
